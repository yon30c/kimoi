...
//   final SUBTITLE_REGEX = RegExp('''#EXT-X-MEDIA:TYPE=SUBTITLES.*?NAME="(.*?)".*?URI="(.*?)"'''); 
//   final AUDIO_REGEX =  RegExp('''#EXT-X-MEDIA:TYPE=AUDIO.*?NAME="(.*?)".*?URI="(.*?)"''');
...
//             playlistUrl,
//             referer: referer,
//             masterHeadersGen:  MasterHeaderGen(headers: masterHeaders!, referer: referer),
//             videoHeadersGen: MasterHeaderGen(headers: videoHeaders!, referer: referer),
...
//     Future<List<Video>> _extractFromHls(
//         String playlistUrl,
//         {String referer = '',
...
//         final masterPlaylistRes = await client.get( Uri.parse(playlistUrl) ,headers: masterHeaders.generateMasterHeaders());
...
//         final masterBase = "${"https://${playlistHttpUrl.host}${playlistHttpUrl.path}"
//             .substringBeforeLast("/")}/";
